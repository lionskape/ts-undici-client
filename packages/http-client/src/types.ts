...
export type Opaque<TOpaqueExtra extends Record<string, unknown> = {}> = OpaqueBase & TOpaqueExtra;
...
export type HttpResult<T, TOpaqueExtra extends Record<string, unknown> = {}> = HttpResultWithSchemas<
        { 200: T },
        TOpaqueExtra
>;
...
export type HttpClientMachineEvent<
        ...
        TOpaqueExtra extends Record<string, unknown> = {}
> =
        | { type: "REQUEST"; input: HttpRequestOptions<TResponseSchemas, TOpaqueExtra> }
        | { type: "RETRY" }
        | { type: "RESET" }
        | { type: "CANCEL" };
...
> {
        id?: string;
        maxAttempts?: number;
        initialContext?: Partial<HttpClientMachineContext<TResponseSchemas, TOpaqueExtra>>;
