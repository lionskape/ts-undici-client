...
): event is AnyEventObject & {
	data: PreparedRequest<TResponseSchemas, TOpaqueExtra>;
} {
	return (
		isObjectLike((event as { data?: unknown }).data) &&
		"options" in ((event as { data?: Record<string, unknown> }).data ?? {})
	);
}

function isResponseEvent(
	event: AnyEventObject,
): event is AnyEventObject & { data: HttpClientResponseSnapshot } {
	const data = (event as { data?: unknown }).data;
	return (
		isObjectLike(data) &&
		typeof (data as { status?: unknown }).status === "number" &&
		"headers" in (data as Record<string, unknown>)
	);
}
...
>(
	...
	data: HttpResultWithSchemas<TResponseSchemas, TOpaqueExtra>;
} {
	const data = (event as { data?: unknown }).data;
...
>(
	...
	const services: Required<
		HttpClientMachineServices<TResponseSchemas, TOpaqueExtra>
	> = {
		prepareRequest:
			providedServices.prepareRequest ??
			(createNotImplementedService("prepareRequest") as Required<
				HttpClientMachineServices<TResponseSchemas, TOpaqueExtra>
			>["prepareRequest"]),
		dispatchRequest:
			providedServices.dispatchRequest ??
			(createNotImplementedService("dispatchRequest") as Required<
				HttpClientMachineServices<TResponseSchemas, TOpaqueExtra>
			>["dispatchRequest"]),
		validateResponse:
			providedServices.validateResponse ??
			(createNotImplementedService("validateResponse") as Required<
				HttpClientMachineServices<TResponseSchemas, TOpaqueExtra>
			>["validateResponse"]),
		...
		},
		{
			actors: {
				prepareRequest: fromPromise(({ input }) =>
					services.prepareRequest(
						input as HttpClientMachineContext<TResponseSchemas, TOpaqueExtra>,
					),
				),
			...
			actions: {
				queueRequest: assign(({ event }) => {
					...
					const requestEvent = event as HttpClientMachineEvent<
						TResponseSchemas,
						TOpaqueExtra
					> & {
						type: "REQUEST";
				...
				notifyQueued: ({ context, event }) => {
					if (
						...
						resolvedActions.onRequestQueued(
							context as HttpClientMachineContext<
								TResponseSchemas,
								TOpaqueExtra
							>,
							event as HttpClientMachineEvent<TResponseSchemas, TOpaqueExtra>,
				...
				incrementAttempt: assign(({ context }) => {
					const current = context as HttpClientMachineContext<
						TResponseSchemas,
						TOpaqueExtra
					>;
				...
				notifySuccess: ({ context }) => {
					resolvedActions.onSuccess(
						context as HttpClientMachineContext<TResponseSchemas, TOpaqueExtra>,
					);
				},
				notifyFailure: ({ context }) => {
					resolvedActions.onFailure(
						context as HttpClientMachineContext<TResponseSchemas, TOpaqueExtra>,
					);
				},
				notifyCancelled: ({ context }) => {
					resolvedActions.onCancelled(
						context as HttpClientMachineContext<TResponseSchemas, TOpaqueExtra>,
					);
				},
			},
